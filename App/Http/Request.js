...
exports.get = function (opt, callback, err) {
    ...
    request.get('http://'+opt.host+opt.path +'?'+querystring.stringify(opt.data, '&', '='), function (error, response, body) {
        callback(body)
    });
};
...
exports.post = function (opt, callback, err) {
    var request = require('request');
    request.post({url: 'http://'+opt.host+opt.path , form: opt.data}, function (error, response, body) {
        callback(body)
    });
